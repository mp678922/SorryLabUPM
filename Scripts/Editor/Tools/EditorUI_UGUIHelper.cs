...
namespace SorryLab.Editor {
    public class EditorUI_UGUIHelper : EditorWindow {
        ...
        void PrintScript(Component component, List<FieldInfo> fieldInfos) {
            MessageClipboard.Write($"//{component.GetType().Name}");
            ...
            MessageClipboard.Write("void Start() {");
            foreach (FieldInfo i in fieldInfos) {
                string functionName = GetFunctionName(i.name);
                if (i.type == typeof(Button)) {
                    MessageClipboard.Write($"\t{i.name}.onClick.AddListener(OnClick_{functionName});");
                }
                if (i.type == typeof(TMP_InputField)) {
                    MessageClipboard.Write($"\t{i.name}.onEndEdit.AddListener(OnEndEdit_{functionName});");
                }
                if (i.type == typeof(TMP_Dropdown)) {
                    MessageClipboard.Write($"\t{i.name}.onValueChanged.AddListener(OnValueChanged_{functionName});");
                }
            ...
            MessageClipboard.Write("}");
            foreach (FieldInfo i in fieldInfos) {
                string functionName = GetFunctionName(i.name);
                if (i.type == typeof(Button)) {
                    MessageClipboard.Write($"private void OnClick_{functionName}() {{ }}");
                }
                if (i.type == typeof(TMP_InputField)) {
                    MessageClipboard.Write($"private void OnEndEdit_{functionName}(string value) {{ }}");
                }
                if (i.type == typeof(TMP_Dropdown)) {
                    MessageClipboard.Write($"private void OnValueChanged_{functionName}(int value) {{ }}");
                }
                if (i.type == typeof(Toggle)) {
                    MessageClipboard.Write($"private void OnValueChanged_{functionName}(bool value) {{ }}");
                }
                if (i.type == typeof(Slider)) {
                    MessageClipboard.Write($"private void OnValueChanged_{functionName}(float value) {{ }}");
                }
            }
            MessageClipboard.Apply();
        }

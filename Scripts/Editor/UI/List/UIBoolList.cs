...
namespace SorryLab.Editor.UI {
    public class UIBoolList : UIElement {
        ReorderableList reorderableList;
        private List<bool> _list = new List<bool>();
        public UIBoolList(string label, List<bool> list) {
            _label = label;
            _list = list;
            ...
            reorderableList.drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) => {
                _list[index] = EditorGUI.Toggle(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight), _list[index]);
            };
        }

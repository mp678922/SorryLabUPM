...
namespace SorryLab.Editor.UI {
    public class UIIntList : UIElement {
        ReorderableList reorderableList;
        private List<int> _list = new List<int>();
        public UIIntList(string label, List<int> list) {
            _label = label;
            _list = list;
            ...
            reorderableList.drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) => {
                _list[index] = EditorGUI.IntField(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight), _list[index]);
            };
        }
